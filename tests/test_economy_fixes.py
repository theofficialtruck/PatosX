...
async def test_process_shop_refund_custom_item_success(monkeypatch):
    ...
    guild_item = {'_id': '123-mystery_box', 'guild': '123', 'name': 'Mystery Box', 'name_lower': 'mystery box', 'price': 500}
...
async def test_fish_resets_cooldown_when_tool_missing(monkeypatch):
    ...
    async def fake_send(message):
        sent.append(message)
    reset_cooldown = Mock()
    ctx = SimpleNamespace(guild=SimpleNamespace(id=123), author=SimpleNamespace(id=42), command=SimpleNamespace(name='fish', reset_cooldown=reset_cooldown), send=AsyncMock(side_effect=fake_send), interaction=None)
