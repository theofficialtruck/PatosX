...
def test_bandit_no_b110_in_main():
    ...
    result = _run(
        ...
        "-r", "main.py",
        "-t", "B110",          # only check this specific test
        "-f", "json",
        "--quiet",
    )
    ...
    assert b110 == [], (
        f"Found {len(b110)} B110 (bare except: pass) violation(s) in main.py:\n"
        + "\n".join(
            f"  line {r['line_number']}: {r['code'].strip()!r}"
            for r in b110
        )
    )
...
def test_bandit_no_b110_in_tests():
    ...
    b110 = [r for r in issues if r.get("test_id") == "B110"]
    assert b110 == [], (
        f"Found {len(b110)} B110 violation(s) in tests/:\n"
        + "\n".join(
            f"  {r['filename']}:{r['line_number']}: {r['code'].strip()!r}"
            for r in b110
        )
    )
...
def test_ruff_no_errors_main():
    ...
    assert errors == [], (
        f"ruff reported {len(errors)} error(s) in main.py:\n"
        + "\n".join(
...
def test_ruff_no_errors_tests():
    ...
    errors = [d for d in diagnostics if d.get("code")]
    assert errors == [], (
        f"ruff reported {len(errors)} error(s) in tests/:\n"
        + "\n".join(
...
def test_codespell_no_spelling_errors():
    ...
    assert result.returncode == 0, (
        "codespell found spelling errors:\n"
