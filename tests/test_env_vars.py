...
def test_beg_donors_is_a_list():
    assert isinstance(cfg.BEG_DONORS, list)


def test_beg_donors_nonempty():
    assert len(cfg.BEG_DONORS) >= 1


def test_beg_donors_contains_cutebatak():
    assert "CuteBatak" in cfg.BEG_DONORS, "CuteBatak should be a beg donor"
...
def test_beg_donors_default_includes_thetruck():
    ...
    assert "thetruck" in cfg.BEG_DONORS, "thetruck must always be a beg donor"
...
def test_beg_donor_default_contains_both_donors():
    ...
    assert "thetruck" in cfg.BEG_DONORS
    assert "CuteBatak" in cfg.BEG_DONORS
