...
async def test_process_shop_purchase_role_item_grants_role_without_inventory(monkeypatch):
    ...
    store_item = {'_id': '123-pond royalty+', 'name': 'Pond Royalty+', 'name_lower': 'pond royalty+', 'price': 15000000, 'role_id': 555}
...
async def test_buy_role_item_grants_role_instead_of_inventory(monkeypatch):
    ...
    sent_messages = []
    store_item = {'_id': '123-pond royalty+', 'guild': '123', 'name': 'Pond Royalty+', 'name_lower': 'pond royalty+', 'price': 15000000, 'description': 'Exclusive premium role for the rich of the rich', 'role_id': 555}
    ...
    async def fake_get_user(*args, **kwargs):
        return {'wallet': 20000000, 'inventory': []}
...
async def test_shop_dropdown_role_item_grants_role(monkeypatch):
    ...
    interaction = SimpleNamespace(user=user, guild=guild, response=response, followup=followup, message=SimpleNamespace(id=999))
    economy = FakeEconomyCol()
    monkeypatch.setattr(main, 'get_user', fake_get_user)
    monkeypatch.setattr(main, 'economy_col', economy)
...
async def test_process_shop_purchase_durable_tool_gets_uses_left(monkeypatch):
    ...
    assert economy.calls == [({'_id': '123-42'}, {'$set': {'wallet': 850, 'inventory': [{'_id': 'fishing rod', 'uses_left': main.TOOL_DURABILITIES['fishing rod']}]}}, False)]