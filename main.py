...
async def check_and_award_badges(
    ...
    try:
        ...
        for badge_id in newly_earned:
            ...
            # Assign or create the badge role
            try:
...
async def check_expired_mutes():
    # Only run if there is a muted person
    count = await mutes_col.count_documents({})
    if count == 0:
        # Also check mod_col just in case
    ...
    # Check mod_col (old system/fallback)
...
async def check_muted_role_permissions():
    for guild in bot.guilds:
        ...
        for channel in guild.channels:
            ...
            # Check if permissions are already set correctly to restrict
            # If any of these are not False, we need to update
            needs_update = False
...
async def coinflip(ctx, amount: str):
    ...
    if won:
        ...
        await badges_col.update_one(
            {"_id": f"{ctx.guild.id}-{ctx.author.id}"},
            {"$set": {"counters.coinflip_win_streak": 0}},
            upsert=True,
        )
