...
from discord import ButtonStyle, ui, NotFound, app_commands, SelectOption, VerificationLevel, AllowedMentions, Embed, File
...
_BEG_DONORS_RAW = os.getenv('BEG_DONORS', 'thetruck')
BEG_DONORS: list = [d.strip() for d in _BEG_DONORS_RAW.split(',') if d.strip()] or ['thetruck']
...
deep_ocean_fishes = [('🐟 Anglerfish', 650), ('🐙 Dumbo Octopus', 900), ('🦑 Giant Squid', 850), ('🦀 King Crab', 800), ('🦞 Spiny Lobster', 750), ('🐡 Blobfish', 600), ('🦈 Goblin Shark', 1100)]
dig_rocks = [('amber shard', 240), ('moonstone fragment', 650), ('fossil core', 1000)]
bugs_to_catch = [('🦋 butterfly', 180), ('🐞 ladybug', 140), ('🐛 caterpillar', 120), ('🪲 beetle', 210), ('🦟 mosquito', 90), ('🪰 fly', 100), ('🕷️ spider', 160), ('🦗 cricket', 150)]
TOOL_DURABILITIES = {'shovel': 336, 'laptop': 28, 'fishing rod': 112, 'scuba gear': 112, 'lockpick': 14, 'pickaxe': 336, 'rifle': 336, 'butterfly net': 336}
NUM_Q = 10
PASS_PCT = 80.0
...
async def check_maintenance_access(ctx):
    ...
    if await is_staff_user(ctx):
        return True
    embed = discord.Embed(title='🔧 Bot Under Maintenance', description='The bot is currently in maintenance mode. Only staff can use commands at this time.', color=discord.Color.orange())
    embed.add_field(name='⏰ Estimated Downtime', value='Maintenance mode will be disabled when staff complete their work.', inline=False)
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CheckFailure):
        ...
        recent_errors.append({'command': ctx.command.name if ctx.command else 'unknown', 'error': str(root_error), 'traceback': traceback.format_exc(), 'time': datetime.now()})
...
async def on_app_command_error(interaction: discord.Interaction, error):
    if isinstance(error, app_commands.CommandNotFound):
        ...
        if similar:
            similar_text = '\n'.join((f'• `{cmd}`' for cmd in similar))
            embed = discord.Embed(title='⚠️ Command Not Found', description=f'Command `/{interaction.command.name}` not found.\n\n**Did you mean:**\n{similar_text}\n\nUse `/help` to see all available commands.', color=discord.Color.orange())
        else:
            embed = discord.Embed(title='⚠️ Command Not Found', description=f'Command `/{interaction.command.name}` not found.\n\nUse `/help` to see all available commands.', color=discord.Color.orange())
    elif isinstance(error, app_commands.MissingRequiredArgument):
        command_name = interaction.command.name if interaction.command else 'unknown'
        syntax = get_command_syntax(command_name)
        embed = discord.Embed(title='⚠️ Missing Required Argument', description=f'**Usage:** {syntax}', color=discord.Color.orange())
    elif isinstance(error, app_commands.BadArgument):
        command_name = interaction.command.name if interaction.command else 'unknown'
        syntax = get_command_syntax(command_name)
        embed = discord.Embed(title='⚠️ Invalid Argument', description=f'**Usage:** {syntax}', color=discord.Color.orange())
...
async def check_disabled(ctx):
    ...
    if category and category in doc.get('disabled_categories', []):
        return False
    return True
STAFF_HELP_COMMANDS = {'kick', 'ban', 'unban', 'mute', 'unmute', 'warn', 'clearwarns', 'purge', 'slowmode', 'blacklist', 'whitelist', 'ticketsetup', 'ticketdeletepanel', 'ticketlist', 'ticketforceclose', 'transcriptsearch', 'transcriptlist', 'ticketaddbutton', 'ticketeditbutton', 'ticketpanel', 'transcript', 'ticketadduser', 'ticketremoveuser', 'ticketsync', 'stickynote', 'unstickynote', 'additem', 'edititem', 'delitem', 'addmoney', 'removemoney', 'reseteconomy', 'investmigrate', 'vanityroles', 'promoters', 'resetpromoters', 'roleadd', 'roleremove', 'configure', 'viewconfig', 'editconfig', 'resetconfig', 'setprefix', 'invitechannel', 'invites', 'removeinvites', 'resetinvites', 'giveaway', 'reroll', 'draw', 'disable', 'enable', 'listdisabled', 'modview', 'say', 'maintenance', 'staff', 'unstaff', 'viewperms', 'debug', 'stop', 'testwelcome', 'testboost', 'reactionrole', 'onetime', 'restore', 'disableonetime', 'performance'}
ECONOMY_HELP_COMMANDS = {'balance', 'daily', 'beg', 'deposit', 'withdraw', 'shop', 'buy', 'use', 'inventory', 'give', 'drop', 'leaderboard', 'badges', 'coinflip', 'duckroll', 'lottery', 'choosejob', 'work', 'jobstatus', 'fish', 'swim', 'rob', 'crime', 'passive', 'sell', 'invest', 'investstatus', 'hunt', 'mine', 'dig', 'bugcatch', 'doorgame', 'ducktowers', 'mines'}
HELP_EXCLUDED_COMMANDS = {'override'}
REMOVED_SHOP_ITEM_KEYS = {'mystery box', 'mystery_box', 'mysterybox'}
BADGES = {'first_cast': {'name': 'First Cast', 'emoji': '🎣', 'description': 'Went fishing for the first time.', 'check': lambda data, xp, extra: extra.get('fish_count', 0) >= 1}, 'fish_fanatic': {'name': 'Fish Fanatic', 'emoji': '🐟', 'description': 'Caught 50 fish total.', 'check': lambda data, xp, extra: extra.get('fish_count', 0) >= 50}, 'first_strike': {'name': 'First Strike', 'emoji': '⛏️', 'description': 'Went mining for the first time.', 'check': lambda data, xp, extra: extra.get('mine_count', 0) >= 1}, 'deep_miner': {'name': 'Deep Miner', 'emoji': '💎', 'description': 'Completed 50 mining trips.', 'check': lambda data, xp, extra: extra.get('mine_count', 0) >= 50}, 'first_hunt': {'name': 'First Hunt', 'emoji': '🔫', 'description': 'Went hunting for the first time.', 'check': lambda data, xp, extra: extra.get('hunt_count', 0) >= 1}, 'big_game_hunter': {'name': 'Big Game Hunter', 'emoji': '🦌', 'description': 'Completed 50 hunts.', 'check': lambda data, xp, extra: extra.get('hunt_count', 0) >= 50}, 'coin_tosser': {'name': 'Coin Tosser', 'emoji': '🪙', 'description': 'Won a coinflip.', 'check': lambda data, xp, extra: extra.get('coinflip_wins', 0) >= 1}, 'hot_streak': {'name': 'Hot Streak', 'emoji': '🔥', 'description': 'Won 10 coinflips in a row.', 'check': lambda data, xp, extra: extra.get('coinflip_win_streak', 0) >= 10}, 'pocket_change': {'name': 'Pocket Change', 'emoji': '💵', 'description': 'Accumulated 1,000 coins.', 'check': lambda data, xp, extra: data.get('wallet', 0) + data.get('bank', 0) >= 1000}, 'millionaire': {'name': 'Millionaire', 'emoji': '🤑', 'description': 'Accumulated 1,000,000 coins.', 'check': lambda data, xp, extra: data.get('wallet', 0) + data.get('bank', 0) >= 1000000}, 'daily_devotee': {'name': 'Daily Devotee', 'emoji': '📅', 'description': 'Reached a 7-day daily streak.', 'check': lambda data, xp, extra: data.get('daily_streak', 0) >= 7}, 'streak_master': {'name': 'Streak Master', 'emoji': '🏆', 'description': 'Reached a 30-day daily streak.', 'check': lambda data, xp, extra: data.get('daily_streak', 0) >= 30}, 'apprentice': {'name': 'Apprentice', 'emoji': '⭐', 'description': 'Earned 500 XP.', 'check': lambda data, xp, extra: xp >= 500}, 'scholar': {'name': 'Scholar', 'emoji': '🎓', 'description': 'Earned 5,000 XP.', 'check': lambda data, xp, extra: xp >= 5000}, 'legend': {'name': 'Legend', 'emoji': '👑', 'description': 'Earned 25,000 XP.', 'check': lambda data, xp, extra: xp >= 25000}, 'shopaholic': {'name': 'Shopaholic', 'emoji': '🛍️', 'description': 'Purchased 10 items from the shop.', 'check': lambda data, xp, extra: extra.get('shop_purchases', 0) >= 10}, 'bug_hunter': {'name': 'Bug Hunter', 'emoji': '🦋', 'description': 'Caught 20 bugs.', 'check': lambda data, xp, extra: extra.get('bug_count', 0) >= 20}, 'treasure_hunter': {'name': 'Treasure Hunter', 'emoji': '🗺️', 'description': 'Dug up 20 rocks.', 'check': lambda data, xp, extra: extra.get('dig_count', 0) >= 20}, 'banker': {'name': 'Banker', 'emoji': '🏦', 'description': 'Deposited at least 10,000 coins into the bank.', 'check': lambda data, xp, extra: data.get('bank', 0) >= 10000}, 'duck_whisperer': {'name': 'Duck Whisperer', 'emoji': '🦆', 'description': 'Owned a Pet Duck.', 'check': lambda data, xp, extra: any((isinstance(i, dict) and i.get('_id') == 'pet_duck' or i == 'pet_duck' for i in data.get('inventory', [])))}}
...
async def check_and_award_badges(ctx_or_channel, guild: discord.Guild, member: discord.Member, economy_data: dict) -> None:
    ...
    try:
        ...
        await badges_col.update_one({'_id': key}, {'$set': {'earned': list(earned_ids), 'guild': guild_id, 'user': user_id, 'counters': extra}}, upsert=True)
...
async def increment_badge_counter(guild_id: str, user_id: str, counter: str, amount: int=1) -> None:
    ...
    key = f'{guild_id}-{user_id}'
    try:
        await badges_col.update_one({'_id': key}, {'$inc': {f'counters.{counter}': amount}, '$set': {'guild': guild_id, 'user': user_id}}, upsert=True)
    except Exception:
        return
        ...
        async def wrapper(ctx, *args, **kwargs):
            ...
            def looks_like_failure_message(content: str) -> bool:
                ...
                if text.startswith(('❌', '⚠️', '⏰', '🕒')):
                    return True
                failure_markers = (' on cooldown', 'try again in', 'you need ', "don't need", "you don't have", 'you cannot', "you can't", 'invalid', 'not found')
            ...
            if guild:
                ...
                key = f'{guild_id}-{user_id}'
                await xp_col.update_one({'_id': key}, {'$inc': {'xp': xp_gained}, '$set': {'guild': guild_id, 'user': user_id}}, upsert=True)
                try:
...
async def get_user(ctx, guild_id, user_id):
    ...
    user_id = str(user_id)
    defaults = {'_id': key, 'guild': guild_id, 'user': user_id, 'wallet': 0, 'bank': 0, 'inventory': [], 'job': None, 'job_start': None, 'promoted': False, 'last_beg': None, 'last_fished': None, 'last_daily': None, 'daily_streak': 0}
...
async def purge_removed_shop_items(guild_id: str | None=None):
    ...
    await shop_col.delete_many({'$or': [{'name_lower': {'$in': list(removed_space)}}, {'_id': {'$in': list(removed_variants)}}]})
    guild_query = {'$or': [{'name_lower': {'$in': list(removed_space)}}, {'_id': {'$regex': '-(?:mystery box|mystery_box|mysterybox)$', '$options': 'i'}}]}
    if guild_id:
...
async def log_action(ctx, message, user_id=None, action_type=None):
    try:
        ...
        if log_channel_id:
            ...
            if log_channel:
                embed = discord.Embed(title='📋 Moderation Log', description=message, color=discord.Color.dark_blue(), timestamp=datetime.now(timezone.utc))
                embed.set_footer(text=f'By {ctx.author} • {ctx.author.id}')
                await log_channel.send(embed=embed)
        if user_id and action_type:
            log_doc = {'guild': guild_id, 'user_id': str(user_id), 'action': action_type, 'by': {'name': str(ctx.author), 'id': str(ctx.author.id)}, 'message': message, 'timestamp': datetime.now(timezone.utc).isoformat()}
    ...
    for guild in bot.guilds:
        ...
        for channel in guild.channels:
            ...
            if isinstance(channel, discord.TextChannel):
                if perms.send_messages is not False or perms.add_reactions is not False or perms.create_public_threads is not False:
                    needs_update = True
            ...
            if needs_update:
                try:
                    if isinstance(channel, discord.TextChannel):
                        await channel.set_permissions(mute_role, send_messages=False, add_reactions=False, create_public_threads=False, create_private_threads=False, send_messages_in_threads=False)
                    elif isinstance(channel, discord.VoiceChannel):
...
async def editconfig(ctx, *, args: str=None):
    ...
    if setting_norm not in valid_settings:
        ...
        embed = discord.Embed(title='⚙️ Invalid Setting', description=f'❌ **`{raw_setting}`** is not a valid configuration key.\n\n**Available settings:**\n' + pretty_list, color=discord.Color.red())
    ...
    try:
        if canonical_key in ['welcome_message', 'boost_message'] and (not value):
            ...
            if canonical_key == 'boost_message':
                await ctx.send('✨ Would you like me to react to each boost message with a custom emoji?\nReact to **this message** with the emoji you want, or type `none` to skip.')
...
async def find_sticky_note_doc(guild_id: int, channel_id: int):
    ...
    return await sticky_col.find_one({'$or': [{'guild': guild_str, 'channel': channel_str}, {'guild': guild_id, 'channel': channel_id}, {'guild': guild_str, 'channel': channel_id}, {'guild': guild_id, 'channel': channel_str}]})
        ...
        async for doc in cursor:
            ...
            if not message_exists:
                try:
                    ...
                    await sticky_col.update_one({'_id': doc['_id']}, {'$set': {'guild': str(guild_id), 'channel': str(channel_id), 'message': sent.id}})
...
def parse_time(duration_str: str) -> int:
    multipliers = {'s': 1, 'sec': 1, 'second': 1, 'seconds': 1, 'm': 60, 'min': 60, 'minute': 60, 'minutes': 60, 'h': 3600, 'hr': 3600, 'hour': 3600, 'hours': 3600, 'd': 86400, 'day': 86400, 'days': 86400, 'w': 604800, 'week': 604800, 'weeks': 604800, 'mo': 2592000, 'month': 2592000, 'months': 2592000, 'y': 31536000, 'yr': 31536000, 'year': 31536000, 'years': 31536000}
...
WORDS_TO_NUM = {'zero': 0, 'one': 1, 'two': 2, 'three': 3, 'four': 4, 'five': 5, 'six': 6, 'seven': 7, 'eight': 8, 'nine': 9, 'ten': 10, 'eleven': 11, 'twelve': 12, 'thirteen': 13, 'fourteen': 14, 'fifteen': 15, 'sixteen': 16, 'seventeen': 17, 'eighteen': 18, 'nineteen': 19, 'twenty': 20, 'thirty': 30, 'forty': 40, 'fifty': 50, 'sixty': 60, 'seventy': 70, 'eighty': 80, 'ninety': 90, 'hundred': 100, 'thousand': 1000, 'million': 1000000, 'billion': 1000000000, 'trillion': 1000000000000}
...
async def ensure_shop_items():
    ...
    initial_items = [{'_id': 'fishing rod', 'name': 'Fishing Rod', 'name_lower': 'fishing rod', 'price': 150, 'description': f"🎣 Needed to catch fish to earn coins. Breaks after {TOOL_DURABILITIES['fishing rod']} uses."}, {'_id': 'scuba gear', 'name': 'Scuba Gear', 'name_lower': 'scuba gear', 'price': 750, 'description': f"🤿 Needed to swim for exotic deep ocean fish. Breaks after {TOOL_DURABILITIES['scuba gear']} uses."}, {'_id': 'laptop', 'name': 'Laptop', 'name_lower': 'laptop', 'price': 500, 'description': f"💻 Needed to work the developer job. Breaks after {TOOL_DURABILITIES['laptop']} uses."}, {'_id': 'pickaxe', 'name': 'Pickaxe', 'name_lower': 'pickaxe', 'price': 500, 'description': f"⛏️ Needed to go mining. Breaks after {TOOL_DURABILITIES['pickaxe']} uses."}, {'_id': 'shovel', 'name': 'Shovel', 'name_lower': 'shovel', 'price': 350, 'description': f"🪏 Needed to dig for cool rocks. Breaks after {TOOL_DURABILITIES['shovel']} uses."}, {'_id': 'rifle', 'name': 'Rifle', 'name_lower': 'rifle', 'price': 500, 'description': f"🔫 Needed to go hunting. Breaks after {TOOL_DURABILITIES['rifle']} uses."}, {'_id': 'lockpick', 'name': 'Lockpick', 'name_lower': 'lockpick', 'price': 250, 'description': f"🗝️ Needed for bank crimes. Breaks after {TOOL_DURABILITIES['lockpick']} uses."}, {'_id': 'butterfly net', 'name': 'Butterfly Net', 'name_lower': 'butterfly net', 'price': 450, 'description': f"🦋 Needed to catch bugs with bugcatch. Breaks after {TOOL_DURABILITIES['butterfly net']} uses."}, {'_id': 'pet_duck', 'name': 'Pet Duck', 'name_lower': 'pet duck', 'price': 1000, 'description': '🦆 Cool pet duck! Gives 30% luck for 3 uses on certain activities.', 'uses_left': 3}, {'_id': 'energy_drink', 'name': 'Energy Drink', 'name_lower': 'energy drink', 'price': 200, 'description': '⚡ Reduces work cooldown by 50% for your next work session. One-time use.', 'uses_left': 1}, {'_id': 'lucky_cookie', 'name': 'Lucky Cookie', 'name_lower': 'lucky cookie', 'price': 150, 'description': '🍪 Doubles your next work/beg earnings. One-time use.', 'uses_left': 1}, {'_id': 'coffee_cup', 'name': 'Coffee Cup', 'name_lower': 'coffee cup', 'price': 100, 'description': '☕ Gives 25% bonus on your next crime success chance. One-time use.', 'uses_left': 1}]
...
class DropClaimView(discord.ui.View):
    ...
    async def claim(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await drop_instances_col.update_one({'message_id': msg_id}, {'$set': {'claimed': True, 'claimer_id': str(interaction.user.id), 'claimed_at': datetime.now(timezone.utc).isoformat()}})
...
async def load_sticky_notes():
    ...
    async for doc in sticky_col.find({}):
        try:
            ...
            await sticky_col.update_one({'_id': doc['_id']}, {'$set': {'guild': str(doc['guild']), 'channel': str(doc['channel']), 'message': new_msg.id}})
...
async def repost_sticky_note(channel_id, guild_id):
    ...
    try:
        ...
        try:
            ...
            pass
        new_msg = await channel.send(doc['text'])
        await sticky_col.update_one({'_id': doc['_id']}, {'$set': {'guild': str(guild_id), 'channel': str(channel_id), 'message': new_msg.id}})
...
async def on_message(message):
        ...
        if message.type in {discord.MessageType.premium_guild_subscription, discord.MessageType.premium_guild_tier_1, discord.MessageType.premium_guild_tier_2, discord.MessageType.premium_guild_tier_3}:
            ...
            if config:
                ...
                if channel and boost_message:
                    ...
                    msg_content = boost_message.replace('{username}', booster.name).replace('{mention}', booster.mention).replace('{server}', guild.name).replace('{boostcount}', str(guild.premium_subscription_count or 0))
                    embed = discord.Embed(description=msg_content, color=discord.Color.fuchsia(), timestamp=datetime.now(timezone.utc))
    ...
    try:
        ...
        if not content.lower().startswith(str(prefix).lower()):
            await config_col.update_one({'guild': guild_id}, {'$setOnInsert': {'quack_count': 0, 'quacks': {}, 'QUACK_CHANNELS': 'all'}}, upsert=True)
            config = await config_col.find_one({'guild': guild_id}) or {}
            ...
            if in_quack_channel and occurrences > 0:
                ...
                await config_col.update_one({'guild': guild_id}, {'$inc': {'quack_count': occurrences, f'quacks.{user_id}': occurrences}})
    except Exception as e:
        print(f'[Quack Counter Error] {e}')
    try:
        guild_id = str(message.guild.id)
        channel_id = str(message.channel.id)
        if guild_id in onetime_channels and channel_id in onetime_channels[guild_id] and (not await has_staff_role(message.author, message.guild)):
            user_id = str(message.author.id)
            if user_id not in onetime_channels[guild_id][channel_id]:
                now = datetime.now(timezone.utc)
                onetime_channels[guild_id][channel_id][user_id] = now
                await settings_col.update_one({'guild': guild_id}, {'$set': {f'onetime_channels.{channel_id}.{user_id}': now}}, upsert=True)
                try:
                    await message.channel.set_permissions(message.author, send_messages=False, reason='One-time message used')
                    await message.channel.send(f'⚠️ {message.author.mention} has used their one-time message in this channel. Staff can restore permissions with `.restore`.')
        ...
        if not (content_lower.startswith('.afk') or content_lower.startswith('/afk')):
            ...
            if doc:
                ...
                try:
                    ...
                    await message.channel.send("⚠️ I couldn't restore your nickname due to role hierarchy, but AFK is removed.", delete_after=5)
                except discord.HTTPException:
                    await message.channel.send('⚠️ Something went wrong while restoring your nickname, but AFK is removed.')
    ...
    try:
        ...
        if doc:
            ...
            await sticky_col.update_one({'_id': doc['_id']}, {'$set': {'guild': str(message.guild.id), 'channel': str(message.channel.id), 'message': sent.id}})
    except Exception as e:
        print(f'[sticky note error] {e}')
    try:
        ticket_entry = await tickets_col.find_one({'guild': str(message.guild.id), 'channel_id': str(message.channel.id), 'close_pending': True})
        if ticket_entry:
    ...
    for guild in bot.guilds:
        ...
        try:
            ...
            for invite in current_invites:
                await invites_col.update_one({'guild_id': str(guild.id), 'code': invite.code}, {'$set': {'inviter_id': str(invite.inviter.id) if invite.inviter else None, 'uses': invite.uses}}, upsert=True)
        except Exception as e:
...
async def run_flake8_lint(base_dir):
    try:
        ...
        process = await asyncio.create_subprocess_exec(sys.executable, '-m', 'flake8', base_dir, '--config', config_path, '--exclude=.venv,__pycache__,build,dist', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        try:
...
async def get_ticket_button_permissions(guild_id: int):
    ...
    options = []
    if categories:
        options.append(SelectOption(label='All Ticket Types', value='tickets:all', description='Access to ALL ticket types'))
    for cat, info in categories.items():
        display = info['label']
        emoji = info['emoji']
        options.append(SelectOption(label=display, value=f'tickets:{cat}', description=f'Access to ticket type: {display}', emoji=emoji))
    return options
...
class StaffPermissionSelect(ui.Select):
    ...
    def __init__(self, member: discord.Member, staffperms_col, guild_id: int, author_id: int, parent_view: ui.View):
        ...
        super().__init__(placeholder='Loading ticket types...', min_values=1, max_values=1, options=[SelectOption(label='Loading...', value='loading')])
        asyncio.create_task(self.load_options())

    async def load_options(self, message: discord.Message=None):
        base_options = [SelectOption(label='Kick', value='kick', description='Use the kick command'), SelectOption(label='Ban', value='ban', description='Use the ban command'), SelectOption(label='Mute', value='mute', description='Use the mute/unmute commands'), SelectOption(label='Stop Bot', value='stopbot', description='Lock the bot from responding'), SelectOption(label='Money Drop', value='money_drop', description='Use the drop command'), SelectOption(label='Other Moderation', value='other_moderation', description='warn / purge / slowmode / fine etc.')]
        ticket_options = [SelectOption(label='Ticket Admin', value='tickets:admin', description='Manage ticket panels and admin actions')]
        categories = {}
        ...
        async for panel in cursor:
            for btn in panel.get('buttons', []):
                ...
                if cat:
                    categories[cat] = {'label': label or cat, 'emoji': emoji}
        if categories:
            ticket_options.append(SelectOption(label='All Ticket Types', value='tickets:all', description='Access to ALL ticket categories'))
            for cat, info in categories.items():
                ticket_options.append(SelectOption(label=info['label'], value=f'tickets:{cat}', description=f"Access to ticket type: {info['label']}", emoji=info['emoji']))
        base_options += ticket_options + [SelectOption(label='StickyNotes', value='stickynotes', description='stickynote / unstickynote'), SelectOption(label='Economy', value='economy', description='shop, addmoney, drop, etc.'), SelectOption(label='Vanity', value='vanity', description='vanityroles, promoters'), SelectOption(label='Roles', value='roles', description='roleadd / claimable roles'), SelectOption(label='Config Changes', value='config', description='configure / editconfig / viewconfig'), SelectOption(label='Invites', value='invites', description='invitechannel / invites / invite removal'), SelectOption(label='Enable/Disable', value='toggle_commands', description='enable/disable/listdisabled'), SelectOption(label='Reaction Roles', value='reactionroles', description='reactionrole management'), SelectOption(label='Giveaways', value='giveaways', description='giveaway / reroll'), SelectOption(label='Give All Permissions', value='all', description='Grant everything')]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await self.staffperms_col.update_one({'guild': str(self.guild_id), 'user': str(self.member.id)}, {'$set': {'permissions': permissions}}, upsert=True)
        embed = discord.Embed(title='Permissions Updated', description=f'{self.member.mention} has been updated:\n{perms_text}\n\nYou can change selections at any time; this menu does not expire.', color=discord.Color.green())
...
async def staff(ctx, member: discord.Member):
    ...
    try:
        ...
        embed = discord.Embed(title='Configure Staff Permissions', description=f'{ctx.author.mention}, use the dropdown below to configure which permission categories or commands\n{member.mention} should have access to. You may select multiple. Choosing **Give All Permissions** will grant everything.\n\nOnly the person who ran this command can use the dropdown. This menu will not expire.', color=discord.Color.blurple())
...
async def get_or_create_blacklist_role(guild: discord.Guild, settings: dict):
    ...
    if role is None:
        role = await guild.create_role(name='Blacklist', colour=discord.Colour(0), reason='Blacklist role created automatically by bot')
...
async def vanityroles(ctx, role: discord.Role, log_channel: discord.TextChannel, keyword: str):
    ...
    await vanity_col.update_one({'guild': guild}, {'$set': {'role': role.id, 'log': log_channel.id, 'keyword': keyword, 'users': []}}, upsert=True)
...
async def resetpromoters(ctx):
    ...
    await ctx.send(embed=discord.Embed(title='🔁 Promoters Reset', description=f'{removed} users removed. List cleared.', color=discord.Color.red()))
...
async def on_presence_update(before, after):
    ...
    if keyword not in status and keyword in new_status and (not has_role):
        ...
        if log_ch:
            await log_ch.send(embed=discord.Embed(title='Vanity Added ✨', description=f'{after.mention} has been awarded **{role.name}** for proudly displaying our vanity `{keyword}` in their status!', color=discord.Color.magenta(), timestamp=datetime.now(timezone.utc)).set_thumbnail(url=after.display_avatar.url))
        ...
        if log_ch:
            await log_ch.send(embed=discord.Embed(title='Vanity Removed', description=f'{after.mention} has lost **{role.name}** for no longer displaying our vanity `{keyword}`.', color=discord.Color.light_gray(), timestamp=datetime.now(timezone.utc)).set_thumbnail(url=after.display_avatar.url))
    ...
    for guild in bot.guilds:
        ...
        for member in guild.members:
            ...
            if keyword in status and (not has_role):
                ...
                if log_ch:
                    await log_ch.send(embed=discord.Embed(title='Vanity Added ✨', description=f'{member.mention} has been awarded **{role.name}**\nFor displaying `{keyword}` in their status!', color=discord.Color.magenta(), timestamp=datetime.now(UTC)).set_thumbnail(url=member.display_avatar.url))
                ...
                if log_ch:
                    await log_ch.send(embed=discord.Embed(title='Vanity Removed', description=f'{member.mention} lost **{role.name}** for no longer displaying `{keyword}` in their status.', color=discord.Color.light_gray(), timestamp=datetime.now(UTC)).set_thumbnail(url=member.display_avatar.url))
                    await repost_sticky_note(log_ch.id, guild.id)
...
async def invitechannel(ctx, channel: discord.TextChannel):
    await invite_config_col.update_one({'guild_id': str(ctx.guild.id)}, {'$set': {'channel_id': str(channel.id)}}, upsert=True)
...
async def removeinvites(ctx, member: discord.Member, amount: int):
    ...
    await invites_col.update_one({'guild_id': guild_id, 'user_id': user_id}, {'$set': {'regular': regular, 'fake': fake, 'leaves': leaves, 'total': new_total}})
...
async def inviteleaderboard(ctx, limit: int=10):
    ...
    for inviter_id, joins in sorted_inv:
        ...
        embed.add_field(name=f'#{rank} {username}', value=f'✅ {joins} joins | ❌ {leaves} leaves → **{total} net**', inline=False)
        rank += 1
    await ctx.send(embed=embed)
button_cooldowns = {}
...
class DoorCountSelect(discord.ui.Select):
    ...
    def __init__(self, ctx, bet):
        ...
        super().__init__(placeholder='Select how many doors to go through...', options=options, min_values=1, max_values=1)
...
class DoorGameButton(discord.ui.View):
    ...
    async def door_clicked(self, interaction: discord.Interaction):
        ...
        if current_time - last_click_time < 2:
            return await interaction.response.send_message(embed=discord.Embed(title='⏳ Cooldown', description='Please wait **2 seconds** before clicking again!', color=16711680), ephemeral=True)
        ...
        next_view = DoorGameButton(self.ctx, self.uid, self.guild_id, self.bet, self.total_doors, next_door, self.current_balance)
        next_embed = discord.Embed(title=f'🚪 Door {next_door}/{self.total_doors}', description='Choose your next door wisely...', color=16753920)
...
def get_towers_stake_multi(layer, difficulty):
    multipliers = {'Easy': [1.1, 1.25, 1.45, 1.7, 2.0], 'Medium': [1.3, 1.6, 2.0, 2.5, 3.2], 'Hard': [1.5, 2.0, 2.8, 4.0, 6.0]}
...
class DifficultySelect(discord.ui.Select):
    ...
    def __init__(self, ctx, bet):
        self.ctx = ctx
        self.bet = bet
        options = [discord.SelectOption(label='Easy', description='Low risk, low reward 🟢'), discord.SelectOption(label='Medium', description='Balanced challenge 🟡'), discord.SelectOption(label='Hard', description='High risk, high reward 🔴')]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title='🦆 **Duck Towers**', description=f'**Difficulty:** {difficulty}\n**Bet:** {add_suffix(self.bet)}\n**Multiplier:** 1.00x → {get_towers_stake_multi(0, difficulty)}x\n**Potential:** {add_suffix(round(self.bet * get_towers_stake_multi(0, difficulty)))}', color=3437035)
...
class DuckTowersView(discord.ui.View):
    ...
    def setup_buttons(self):
        ...
        for layer in range(5):
            ...
            for tower in range(towers_per_layer):
                btn = discord.ui.Button(label='\u200e', custom_id=f'{layer} {tower}', style=discord.ButtonStyle.gray, row=row)
    ...
    async def update_embed(self):
        ...
        embed = discord.Embed(title='🦆 **Duck Towers**', description=f'**Difficulty:** {self.difficulty}\n**Bet:** {add_suffix(self.bet)}\n**Multiplier:** {self.multi}x → {next_multi}x\n**Potential:** {add_suffix(potential)}', color=3437035)
    ...
    async def tower_clicked(self, interaction: discord.Interaction):
        ...
        if not is_safe:
            ...
            lose_embed = discord.Embed(title='💥 Game Over', description=f'**Bet:** {add_suffix(self.bet)}\n**Multiplier:** {self.multi}x\n**Winnings:** 0', color=16711680)
    ...
    async def cash_out(self, interaction: discord.Interaction):
        ...
        for row in self.buttons:
            for b in row:
                b.disabled = True
        embed = discord.Embed(title='💰 Cashed Out!', description=f'**Bet:** {add_suffix(self.bet)}\n**Winnings:** {add_suffix(winnings)}\n**Multiplier:** {self.multi}x', color=65280)
    ...
    try:
        if not member_name:
            ...
            if not member:
                ...
                matches = [m for m in ctx.guild.members if m.display_name.lower().startswith(search_term) or m.name.lower().startswith(search_term)]
                if len(matches) == 0:
...
async def daily(ctx):
    ...
    try:
        ...
        await economy_col.update_one({'_id': f'{ctx.guild.id}-{ctx.author.id}'}, {'$set': {'last_daily': now.isoformat(), 'daily_streak': saved_streak}})
        embed = discord.Embed(title='🎁 Daily Reward Claimed!', description=f'💰 You earned **{reward} coins**!', color=discord.Color.gold(), timestamp=now)
        embed.add_field(name='🔥 Current Streak', value=f'Day **{current_streak}** of 30' if current_streak > 0 else 'First time claiming!', inline=True)
...
async def deposit(ctx, amount: str):
    ...
    try:
        ...
        await economy_col.update_one({'_id': f'{ctx.guild.id}-{ctx.author.id}'}, {'$set': {'wallet': wallet - deposit_amount, 'bank': data['bank'] + taxed_amount}})
...
async def withdraw(ctx, amount: str):
    ...
    try:
        ...
        if amount.lower() == 'all':
            ...
            return await ctx.send('❌ Please enter a valid number or `all`.')
        await economy_col.update_one({'_id': f'{ctx.guild.id}-{ctx.author.id}'}, {'$set': {'wallet': data['wallet'] + withdraw_amount, 'bank': bank - withdraw_amount}})
...
async def process_shop_purchase(member, guild, store_item: dict, user_data: dict):
    ...
    return {'ok': True, 'message': success_message, 'item_name': item_name, 'price': price, 'old_wallet': wallet, 'new_wallet': new_wallet, 'purchase_type': purchase_type}
...
async def process_shop_refund(member, guild, item_key: str, user_data: dict):
    ...
    store_item = await guild_shop_col.find_one({'guild': str(guild.id), '$or': [{'name_lower': {'$in': list(normalized_candidates)}}, {'_id': {'$in': prefixed_ids}}]})
    if not store_item:
    ...
    return {'ok': True, 'item_name': item_name, 'refund_amount': refund_amount, 'old_wallet': wallet, 'new_wallet': new_wallet, 'removed_item': removed_item}
...
class ShopView(discord.ui.View):
    ...
    async def buy_items(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for item in self.items:
            ...
            options.append(discord.SelectOption(label=f'{display_name} - 🪙 {price}', description=f'{description[:50]}...' if len(description) > 50 else description, value=item['_id'], emoji=can_afford))
        view = ShopDropdown(self.user_id, self.guild_id, self.items, self.balance, options)
        embed = discord.Embed(title='🛒 Select Item to Buy', description=f'Your wallet: 🪙 {self.balance:,}\n\nChoose an item from the dropdown below:', color=discord.Color.blue())
    ...
    async def refund_items(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for item_key, count in counts.items():
            ...
            store_item = await guild_shop_col.find_one({'guild': guild_id, '$or': [{'name_lower': {'$in': list(normalized_candidates)}}, {'_id': {'$in': prefixed_ids}}]})
            if not store_item:
                store_item = await shop_col.find_one({'$or': [{'name_lower': {'$in': list(normalized_candidates)}}, {'_id': {'$in': list(normalized_candidates)}}]})
            if not store_item:
                continue
            ...
            options.append(discord.SelectOption(label=label[:100], description=description[:100], value=item_key, emoji='💸'))
        if not options:
            await interaction.response.send_message('❌ No refundable items were found in your inventory.', ephemeral=True)
            return
        view = RefundDropdown(self.user_id, guild_id, options)
        embed = discord.Embed(title='💸 Refund an Item', description='Choose one inventory item to refund for half its shop price.', color=discord.Color.orange())
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
...
class ShopDropdown(discord.ui.View):
    ...
    async def dropdown_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(title='✅ Purchase Successful!', description=f"You bought **{result['item_name']}**!\n\nPrice: 🪙 {result['price']:,}\nOld Wallet: 🪙 {result['old_wallet']:,}\nNew Wallet: 🪙 {result['new_wallet']:,}" + (f"\nRole Granted: {result['role_mention']}" if result['purchase_type'] == 'role' else '\n\nUse `.inventory` to view your items!'), color=discord.Color.green())
            await interaction.response.send_message(embed=embed, ephemeral=True)
...
class RefundDropdown(discord.ui.View):
    ...
    async def dropdown_callback(self, interaction: discord.Interaction):
        ...
        if not result.get('ok'):
            await interaction.response.send_message(result['message'], ephemeral=True)
            return
        embed = discord.Embed(title='✅ Refund Complete', description=f"Refunded **{result['item_name']}**.\n\nReturned: 🪙 {result['refund_amount']:,}\nOld Wallet: 🪙 {result['old_wallet']:,}\nNew Wallet: 🪙 {result['new_wallet']:,}", color=discord.Color.green())
        await interaction.response.send_message(embed=embed, ephemeral=True)
        self.dropdown.disabled = True
...
async def maintenance(ctx, action: str=None):
    ...
    if action is None:
        ...
        embed = discord.Embed(title='🔧 Maintenance Status', description=f"Maintenance mode is currently: {('**ON**' if is_maintenance else '**OFF**')}", color=discord.Color.orange() if is_maintenance else discord.Color.green())
        if is_maintenance:
            embed.add_field(name='⚠️ Current Status', value='• Only staff can use bot commands\n• Channel restrictions are bypassed for staff\n• Regular users cannot use any commands', inline=False)
        else:
            embed.add_field(name='✅ Current Status', value='• All users can use bot commands\n• Channel restrictions are enforced\n• Normal operation mode', inline=False)
    ...
    if action == 'on':
        embed = discord.Embed(title='🔧 Maintenance Mode Enabled', description='**Bot is now in maintenance mode!**', color=discord.Color.orange())
        embed.add_field(name='⚠️ What Changed', value='• Only staff members can use bot commands\n• Channel restrictions are ignored for staff\n• Regular users see maintenance messages', inline=False)
        embed.add_field(name='👤 Who Can Use Commands', value='• Server Owner\n• Staff members (with configured staff role)\n• Users with admin permissions', inline=False)
        embed.set_footer(text='Use `.maintenance off` to disable maintenance mode')
    else:
        embed = discord.Embed(title='✅ Maintenance Mode Disabled', description='**Bot is back to normal operation!**', color=discord.Color.green())
        embed.add_field(name='🔄 What Changed', value='• All users can use bot commands again\n• Channel restrictions are enforced\n• Normal operation resumed', inline=False)
...
async def shop(ctx):
    ...
    try:
        ...
        existing_name_lowers = {str(item.get('name_lower') or '').strip().lower() for item in items_list if item.get('name_lower')}
        if not exists:
        ...
        embed = discord.Embed(title='🛍️ Shop', description=f'💰 Wallet: 🪙 {wallet_balance:,}\n🏦 Bank: 🪙 {bank_balance:,}\n💳 **Total: 🪙 {total_balance:,}**\n\nClick the button below to purchase items! Purchases use wallet coins only.', color=discord.Color.green())
        if items_list:
            for item in items_list:
                ...
                if len(embed.fields) >= 24:
                    embed.add_field(name='🛍️ More Items', value=f'... and {len(items_list) - len(embed.fields) + 1} more items! Use the shop view to see all items.', inline=False)
                    break
        else:
...
async def edititem(ctx, *, name: str):
    ...
    if choice in ['yes', 'y']:
        role_id = await prompt_for_role(ctx)
    await guild_shop_col.update_one({'guild': guild_id, 'name_lower': name.lower()}, {'$set': {'name': new_name, 'name_lower': new_name.lower(), 'price': new_price, 'description': new_desc, 'role_id': role_id}})
...
async def inventory(ctx):
    ...
    if duck_total > 0:
        ...
        embed.add_field(name=f"{shop_item['name']} x{duck_total}", value=f"{shop_item.get('description', '')} ({duck_uses} uses left total)", inline=False)
    for tool_key, durability_values in tool_durability.items():
        ...
        if len(durability_values) > 5:
            durability_text += ', ...'
        embed.add_field(name=f'{display_name} x{len(durability_values)}', value=f'{description}\nDurability: {durability_text}', inline=False)
        if len(embed.fields) >= 24:
            break
    if len(embed.fields) >= 24:
        return await ctx.send(embed=embed)
    for key, count in counts.items():
        ...
        if shop_item:
            embed.add_field(name=f"{shop_item['name']} x{count}", value=f"{shop_item.get('description', 'No description.')}", inline=False)
        else:
            clean_name = key.split('-', 1)[-1] if '-' in key else key
            emoji = '📦'
            embed.add_field(name=f"{emoji} {clean_name.replace('_', ' ').title()} x{count}", value='*Item no longer sold in shop*', inline=False)
        if len(embed.fields) >= 24:
            embed.add_field(name='📦 More Items', value=f'... and {len(counts) - len(embed.fields) + 1} more items! Use `.inventory` again to see details.', inline=False)
            break
    await ctx.send(embed=embed)
...
async def coinflip(ctx, amount: str):
    ...
    if won:
        ...
        await badges_col.update_one({'_id': f'{ctx.guild.id}-{ctx.author.id}'}, {'$set': {'counters.coinflip_win_streak': 0}}, upsert=True)
    fresh_data = await get_user(ctx, ctx.guild.id, ctx.author.id)
    await check_and_award_badges(ctx, ctx.guild, ctx.author, fresh_data)
...
class JobPicker(ui.View):
    ...
    async def set_job(self, interaction, job_name):
        await economy_col.update_one({'_id': f'{self.ctx.guild.id}-{self.ctx.author.id}'}, {'$set': {'job': job_name, 'job_start': datetime.now(timezone.utc).isoformat(), 'promotion_level': 0, 'promotion_chance': 20.0, 'last_promo_check': None}}, upsert=True)
        await interaction.response.edit_message(content=f'✅ You are now working as a **{job_name.capitalize()}**!', view=None)
...
async def reseteconomy(ctx):
    ...
    try:
        ...
        await settings_col.update_one({'guild': str(ctx.guild.id)}, {'$set': {'season_reset_time': datetime.now(UTC)}}, upsert=True)
        ...
        if days >= 7:
            ...
            if not promoted:
                ...
                if allow_roll:
                    ...
                    embed.add_field(name='Promotion Chance', value=f'⏳ On cooldown ({promo_chance:.2f}%) - next roll <t:{int(next_time.timestamp())}:f>', inline=False)
                await ctx.send(embed=embed)
        else:
            embed = discord.Embed(title=f'📋 Job Status for {ctx.author.display_name}', color=discord.Color.blue())
            embed.add_field(name='Job', value=job.capitalize(), inline=False)
...
async def fish(ctx):
    ...
    try:
        ...
        await economy_col.update_one({'_id': user_id}, {'$set': {'inventory': inventory, 'last_fished': now.isoformat()}})
...
async def crime(ctx, *, choice: str):
    ...
    try:
        ...
        config = {'bank': {'chance': 0.4, 'gain': (1200, 3000), 'fine': (600, 1500)}, 'shoplift': {'chance': 0.5, 'gain': (300, 600), 'fine': (150, 400)}, 'payroll': {'chance': 0.4, 'gain': (800, 1500), 'fine': (400, 800)}}
        conf = config[choice]
        luck_buff = 0.0
        for i, item in enumerate(inventory):
            if isinstance(item, dict) and item.get('_id') == 'pet_duck':
        ...
        if success:
            ...
            await add_balance(ctx.author.id, ctx.guild.id, amount)
            await economy_col.update_one({'_id': f'{ctx.guild.id}-{ctx.author.id}'}, {'$set': {'inventory': inventory, 'last_crime': now.isoformat()}})
...
async def passive(ctx, member: discord.Member=None):
    ...
    if passive_until:
        ...
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=timezone.utc)
        if dt > now:
            await economy_col.update_one({'_id': user_id}, {'$unset': {'passive_until': ''}, '$set': {'last_passive_toggle': now.isoformat()}})
    ...
    await economy_col.update_one({'_id': user_id}, {'$set': {'passive_until': until_time.isoformat(), 'last_passive_toggle': now.isoformat()}}, upsert=True)
    if target == ctx.author:
...
async def sell(ctx, *, item: str=None):
    ...
    try:
        ...
        total_gain = 0
        sold_items = []
        prices = {'rabbit': 200, 'deer': 450, 'bear': 600, 'fish': 150, 'iron ore': 200, 'gold ore': 500, 'diamond': 1200, 'amber shard': 240, 'moonstone fragment': 650, 'fossil core': 1000}
        if item_name == 'all':
            confirm_view = ConfirmSellAll(ctx, prices, inventory, user_id, wallet)
            confirm_embed = discord.Embed(title='⚠️ Confirm Sell All', description='You are about to sell **ALL ores, hunted animals, and investments.**\n\nThis includes:\n• Rabbits, deer, bears, fish, ores, diamonds, dig rocks\n• All company investments\n\nAre you sure you want to continue?', color=discord.Color.red())
            ...
            await economy_col.update_one({'_id': user_id}, {'$set': {'wallet': wallet + total_gain, 'inventory': inventory}})
...
async def create_investment(user_id: str, company: str, amount: int):
    ...
    await investments_col.insert_one({'_id': inv_id, 'user_id': user_id, 'company': company, 'amount': amount, 'current_value': amount, 'last_status_refresh_date': now.date().isoformat(), 'date': now_iso, 'timestamp': now_iso, 'history': []})
    ...
    for inv in investments:
        ...
        if days_elapsed <= 0:
            if inv.get('last_status_refresh_date') != refresh_date:
                await investments_col.update_one({'_id': inv['_id']}, {'$set': {'last_status_refresh_date': refresh_date}})
...
async def invest(ctx, company: str=None, amount: str=None):
    if not await check_channel(ctx, 'economy_channel', 'Economy'):
        return
    companies = {'Techify': {'min': 500, 'max': 5000}, 'MineCorp': {'min': 300, 'max': 3000}, 'Oceanic': {'min': 200, 'max': 2500}}
    if company and amount:
        ...
        if user_investments >= 5:
            return await ctx.send('❌ You can only have up to **5 active investments** at a time. Sell some before investing again.')
        if amount.lower() == 'all':
        ...
        return
    embed = discord.Embed(title='📈 Investment Opportunities', description='Choose a company to invest in!', color=discord.Color.green())
    ...
    for company, stats in companies.items():
        ...
        async def button_callback(interaction, company=company, stats=stats):
            ...
            user_investments = await investments_col.count_documents({'user_id': user_id})
            if user_investments >= 5:
                return await interaction.response.send_message('❌ You can only have up to **5 active investments** at a time. Sell some before investing again.', ephemeral=True)
            if wallet < stats['min']:
                return await interaction.response.send_message(f"❌ You need at least {stats['min']} coins to invest in {company}.", ephemeral=True)
            step = 500
            ...
            async def select_callback(inter: discord.Interaction):
                ...
                if latest_wallet < invest_amount:
                    return await inter.response.send_message(f'❌ You only have {latest_wallet} coins but tried to invest {invest_amount}.', ephemeral=True)
                update_result = await economy_col.update_one({'_id': user_id, 'wallet': {'$gte': invest_amount}}, {'$inc': {'wallet': -invest_amount}}, upsert=False)
                if getattr(update_result, 'modified_count', 0) == 0:
                    fresh_data = await get_user(ctx, ctx.guild.id, ctx.author.id)
                    fresh_wallet = int(fresh_data.get('wallet', 0) or 0)
                    return await inter.response.send_message(f'❌ You only have {fresh_wallet} coins but tried to invest {invest_amount}.', ephemeral=True)
                await create_investment(user_id, company, invest_amount)
                await inter.response.send_message(f'✅ You invested **{invest_amount} coins** in **{company}**.')
            select.callback = select_callback
            await interaction.response.send_message(f'💰 Choose how much to invest in **{company}**:', view=View().add_item(select), ephemeral=True)
...
async def investstatus(ctx):
    ...
    for inv in investments:
        ...
        embed.add_field(name=f'{company} (ID: {inv_id})', value=f'Invested: 🪙 {amount}\nCurrent Value: 🪙 {current_value}\nDate: <t:{unix_timestamp}:F>', inline=False)
    await ctx.send(embed=embed)
...
class AnswerButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        reply = '✅ Correct!' if self.value == correct_answer else f"❌ Wrong! Answer was: {view.questions[idx]['options'][correct_answer - 1]}"
...
class QuizView(discord.ui.View):
    ...
    async def show_next(self, interaction: discord.Interaction=None):
        ...
        embed = discord.Embed(title=f'Question {self.current_index + 1}/{len(self.questions)}', description=q['q'], color=discord.Color.teal())
    ...
    async def finish_quiz(self, interaction: discord.Interaction=None):
        ...
        passed = pct >= PASS_PCT
        await quiz_col.update_one({'_id': self.quiz_id}, {'$set': {'score': self.score, 'completed': datetime.now(timezone.utc), 'passed': passed}})
...
async def duckquiz(ctx):
    ...
    quiz_doc = {'guild': GUILD, 'user': USER, 'started': now, 'questions': [q['id'] for q in selected], 'answers': {}, 'score': 0, 'completed': None, 'passed': False}
...
class QuackTopView(View):
    ...
    def get_embed(self):
        ...
        embed = discord.Embed(title=f'🦆 Quack Leaderboard (Page {self.page + 1}/{self.max_page + 1})', description=description, color=discord.Color.green())
        if self.user_rank:
...
async def slap(ctx, member: discord.Member=None):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f'https://tenor.googleapis.com/v2/search?q=anime slap&key={TENOR_API_KEY}&limit=20', timeout=5) as r:
                if r.status != 200:
        ...
        embed = discord.Embed(title='👋 Slap!', description=f'{ctx.author.mention} slapped {member.mention}! Ouch!', color=discord.Color.red())
...
async def afk(ctx, *, reason='AFK'):
    ...
    await afk_col.update_one({'_id': afk_key}, {'$set': {'reason': reason, 'timestamp': datetime.now(timezone.utc).isoformat(), 'original_nick': ctx.author.nick}}, upsert=True)
    ...
    try:
        ...
        await ctx.send("⚠️ I can't change your nickname (role hierarchy or missing permissions). AFK still set!", delete_after=5)
    except discord.HTTPException:
...
class TicketSetupModal(discord.ui.Modal, title='Create Ticket Panel'):
    ...
    async def _handle_submit(self, interaction: discord.Interaction):
        ...
        if not staff_role_id or staff_role_id not in [r.id for r in self.ctx.author.roles]:
            embed = discord.Embed(title='❌ Access Denied', description='Only staff members can use this command.', color=discord.Color.red())
            await interaction.followup.send(embed=embed, ephemeral=True)
            return
        color_value = int(self.embed_color.value.replace('#', ''), 16) if self.embed_color.value else 5793266
        await ticket_panels_col.insert_one({'guild': str(guild.id), 'panel_name': self.panel_name.value, 'ticket_embed_title': self.embed_title.value, 'ticket_embed_desc': self.embed_desc.value, 'ticket_embed_color': color_value, 'buttons': []})
        embed = discord.Embed(title='✅ Ticket Panel Created', description=f'Panel `{self.panel_name.value}` created successfully!\nUse `/ticketaddbutton` to add buttons.', color=discord.Color.green())
        await interaction.followup.send(embed=embed, ephemeral=True)
...
class TicketAddButtonModal(discord.ui.Modal, title='Add Ticket Panel Button'):
    ...
    async def _handle_submit(self, interaction: discord.Interaction):
        ...
        if not panel_data:
            await interaction.followup.send(embed=discord.Embed(title='❌ Not Found', description=f'No panel found with name `{self.panel_name.value}`.', color=discord.Color.red()), ephemeral=True)
            return
        new_button = {'category_name': self.category_name.value, 'label': self.button_label.value, 'emoji': self.emoji.value if self.emoji.value else None}
        await ticket_panels_col.update_one({'guild': str(guild.id), 'panel_name': self.panel_name.value}, {'$push': {'buttons': new_button}})
        embed = discord.Embed(title='✅ Button Added', description=f"Added button to panel `{self.panel_name.value}`:\n{self.emoji.value or ''} **{self.button_label.value}** → Category `{self.category_name.value}`", color=discord.Color.green())
        await interaction.followup.send(embed=embed, ephemeral=True)
...
class TicketEditButtonModal(discord.ui.Modal, title='Edit Ticket Panel Button'):
    ...
    async def _handle_submit(self, interaction: discord.Interaction):
        guild = self.ctx.guild
        await ticket_panels_col.update_one({'guild': str(guild.id), 'panel_name': self.panel_name, 'buttons.label': self.btn_data['label']}, {'$set': {'buttons.$.category_name': self.category_name.value, 'buttons.$.label': self.button_label.value, 'buttons.$.emoji': self.emoji.value if self.emoji.value else None}})
        embed = discord.Embed(title='✅ Button Updated', description=f"Updated button in panel `{self.panel_name}`:\n{self.emoji.value or ''} **{self.button_label.value}** → Category `{self.category_name.value}`", color=discord.Color.green())
        await interaction.followup.send(embed=embed, ephemeral=True)
...
class TicketButtonActionView(discord.ui.View):
    ...
    async def _edit(self, interaction: discord.Interaction):
        if interaction.user != self.ctx.author:
            return await interaction.response.send_message('❌ Only the staff member who ran the command can edit.', ephemeral=True)
    ...
    async def _delete(self, interaction: discord.Interaction):
        if interaction.user != self.ctx.author:
            return await interaction.response.send_message('❌ Only the staff member who ran the command can delete.', ephemeral=True)
        guild = self.ctx.guild
        await ticket_panels_col.update_one({'guild': str(guild.id), 'panel_name': self.panel_name}, {'$pull': {'buttons': {'label': self.btn_data['label']}}})
        await interaction.response.send_message(embed=discord.Embed(title='🗑 Button Deleted', description=f"Removed **{self.btn_data['label']}** from panel `{self.panel_name}`.", color=discord.Color.red()), ephemeral=True)
        self.stop()
...
class TicketEditButton(discord.ui.Button):
    ...
    def __init__(self, btn_data, panel_data, ctx):
        ...
        super().__init__(label=btn_data.get('label', 'Unnamed'), emoji=btn_data.get('emoji') or None, style=discord.ButtonStyle.gray, custom_id=f'editbtn_{safe_category}_{safe_label}')
        self.btn_data = btn_data
        self.panel_data = panel_data
        self.ctx = ctx
    ...
    async def _callback(self, interaction: discord.Interaction):
        if interaction.user != self.ctx.author:
            return await interaction.response.send_message('❌ Only the staff member who ran the command can manage buttons.', ephemeral=True)
        await interaction.response.send_message(embed=discord.Embed(title=f"⚙ Manage Button: {self.btn_data.get('label', 'Unnamed')}", description='Choose what you want to do with this button.', color=discord.Color.orange()), view=TicketButtonActionView(self.ctx, self.panel_data['panel_name'], self.btn_data), ephemeral=True)
...
class TicketCategoryButton(discord.ui.Button):
    ...
    def __init__(self, btn_data, panel_data):
        ...
        super().__init__(label=btn_data.get('label', 'Open Ticket'), emoji=btn_data.get('emoji') or None, style=discord.ButtonStyle.green, custom_id=f'ticket_{guild_id}_{panel_name}_{safe_category}_{safe_label}')
        self.btn_data = btn_data
        self.panel_data = panel_data
    ...
    async def create_ticket(self, interaction: discord.Interaction):
        ...
        for c in category.channels:
            if c.name.lower() == ticket_name.lower():
                return await interaction.followup.send(embed=discord.Embed(title='❌ Duplicate Ticket', description=f'A ticket with that name already exists: {c.mention}', color=discord.Color.red()), ephemeral=True)
        ...
        overwrites = {guild.default_role: discord.PermissionOverwrite(view_channel=False, embed_links=True, attach_files=True), author: discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, embed_links=True, attach_files=True)}
        for member in category_support_members:
            overwrites[member] = discord.PermissionOverwrite(view_channel=True, send_messages=True, read_message_history=True, embed_links=True, attach_files=True)
        channel = await guild.create_text_channel(ticket_name, category=category, overwrites=overwrites)
        await tickets_col.insert_one({'guild': str(guild.id), 'channel_id': str(channel.id), 'owner_id': str(author.id), 'category': category_name.lower(), 'created_at': datetime.now(timezone.utc)})
        embed = discord.Embed(title='🎟️ Ticket Created', description='Please state your concern and the staff team will respond soon.', color=discord.Color(self.panel_data.get('ticket_embed_color', 5793266)))
        await channel.send(embed=embed)
        await ping_ticket_roles(channel, guild.id, opener_id=author.id)
        await interaction.followup.send(embed=discord.Embed(title='✅ Ticket Created!', description=f'Your ticket was successfully created!\nHere it is: {channel.mention}', color=discord.Color.green()), ephemeral=True)
...
async def ping_ticket_roles(channel: discord.TextChannel, guild_id: str, opener_id: int=None):
    try:
        ...
        msg = await channel.send(content=ping_text, allowed_mentions=discord.AllowedMentions(users=True, roles=True, everyone=False))
...
async def actually_close_ticket(ctx, opener, forced=False):
    ...
    await tickets_col.insert_one({'ticket_id': ticket_id, 'guild_id': str(channel.guild.id), 'channel_id': str(channel.id), 'opener_id': str(opener.id) if opener else None, 'closer_id': str(ctx.author.id), 'closer_name': str(ctx.author), 'transcript': transcript_text, 'created_at': str(channel.created_at), 'closed_at': datetime.now(timezone.utc).isoformat(), 'forced': forced})
    ...
    await log_action(ctx, f"Ticket `{channel.name}` closed by {closer_text} (opener: {opener_text}){(' [FORCED]' if forced else '')}", user_id=ctx.author.id, action_type=action_type)
    if forced:
...
async def ticketaddbutton(ctx):
    try:
        if is_prefix(ctx):
            ...
            await ticket_panels_col.update_one({'guild': str(guild.id), 'panel_name': panel_name}, {'$push': {'buttons': new_button}})
            return await ctx.send(f"✅ Added button to panel `{panel_name}`:\n{emoji or ''} **{button_label}** → Category **{category_name}**")
    ...
    try:
        ...
        if not panel_data:
            ...
            if ctx.interaction:
                ...
                await ctx.send(msg)
            return
        embed = discord.Embed(title=panel_data.get('ticket_embed_title', '🎫 Ticket Panel'), description=panel_data.get('ticket_embed_desc', 'Click a button below to create a ticket.'), color=discord.Color(int(panel_data.get('ticket_embed_color', 5793266))))
        view = TicketPanelView(panel_data)
        if ctx.interaction:
            msg = await ctx.interaction.response.send_message(embed=embed, view=view)
        else:
            msg = await ctx.send(embed=embed, view=view)
        await ticket_panels_col.update_one({'_id': panel_data['_id']}, {'$set': {'message_id': msg.id, 'channel_id': str(msg.channel.id)}})
    except Exception as e:
    ...
    try:
        ...
        if not panel_data:
            ...
            if ctx.interaction:
                ...
                await ctx.send(msg)
            return
        embed = discord.Embed(title=f'📝 Edit Mode: {panel_name}', description='Click a button below to edit or delete it.', color=discord.Color.orange())
...
async def ticketclose(ctx):
    ...
    async def inner():
        ...
        await send_public(content=f'{opener.mention}, do you confirm closing this ticket? Type `confirm` to close or `cancel` to keep it open. (This will wait until you reply, no time limit.)')
...
async def transcriptsearch(ctx, username: str):
    try:
        query = {'guild_id': str(ctx.guild.id), '$or': [{'opener_name': {'$regex': username, '$options': 'i'}}, {'closer_name': {'$regex': username, '$options': 'i'}}]}
        ...
        for t in tickets:
            embed.add_field(name=t['ticket_id'], value=f"Opened by: <@{t.get('opener_id', 'unknown')}> | Closed by: <@{t.get('closer_id', 'unknown')}>", inline=False)
        if ctx.interaction:
            await ctx.interaction.response.send_message(embed=embed, ephemeral=True)
        else:
            await ctx.send(embed=embed)
...
class TranscriptPaginationView(discord.ui.View):
    ...
    async def build_embed(self):
        ...
        embed = discord.Embed(title=f'📜 Ticket Overview ({len(self.tickets)} total) - Page {self.page + 1}/{self.max_page + 1}', color=discord.Color.blurple())
        for t in chunk:
...
class GiveawayView(discord.ui.View):
    ...
    async def update_db(self):
        await giveaway_col.update_one({'_id': self.giveaway_id}, {'$set': {'participants': {str(uid): entries for uid, entries in self.participants.items()}}})
...
async def on_submit(self, interaction: discord.Interaction):
    try:
        ...
        await polls_col.insert_one({'poll_id': poll_id, 'question': self.question.value, 'options': options, 'votes': {}, 'channel_id': str(post_channel.id), 'message_id': str(msg.id), 'end_time': end_time, 'duration_raw': self.duration.value})
...
async def poll(ctx):
    ...
    try:
        ...
        await polls_col.insert_one({'poll_id': poll_id, 'question': question, 'options': final_options, 'votes': {}, 'channel_id': str(channel.id), 'message_id': str(poll_msg.id), 'end_time': end_time, 'duration_raw': msg.content})
...
class GiveawayModal(discord.ui.Modal, title='Create Giveaway'):
    ...
    bonus_roles = discord.ui.TextInput(label='Bonus Roles (optional)', placeholder='Format: role_id|bonus, role_id|bonus', required=False)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        giveaway_data = {'_id': giveaway_id, 'channel_id': interaction.channel.id, 'message_id': sent_message.id, 'prize': self.prize.value, 'host_id': interaction.user.id, 'end_time': end_time.isoformat(), 'winners_count': winners_count, 'participants': {}, 'ended': False, 'bonus_roles': bonus_roles_dict}
    ...
    async for data in active_giveaways:
        try:
            ...
            if not channel:
                ...
                continue
            message = await channel.fetch_message(data['message_id'])
            view = GiveawayView(embed_message=message, giveaway_id=data['_id'], end_time=end_time, winners=data['winners_count'], prize=data['prize'])
            participants = data.get('participants', {})
            if isinstance(participants, list):
                participants = {}
            for uid, entries in participants.items():
...
async def refresh_roles_embed(ctx, guild_id):
    ...
    if not roles:
        embed = discord.Embed(title='🎭 Claim Your Roles', description='⚠️ No valid roles available.', color=discord.Color.red())
        await msg.edit(embed=embed, view=None)
        return
    embed = discord.Embed(title='🎭 Claim Your Roles', description='\n'.join([role.mention for role in roles]), color=discord.Color.blurple())
...
async def addmoney_error(ctx, error):
    try:
        ...
        if isinstance(error, commands.BadArgument):
            return await send_hybrid_error(ctx, content=f'❌ Invalid arguments. Usage: `{prefix}addmoney <amount> @user`\nExample: `{prefix}addmoney 100 @User`')
        elif isinstance(error, commands.MissingRequiredArgument):
...
async def removemoney(ctx, amount: str, user: discord.Member):
    ...
    await log_action(ctx, f"Removed 🪙 {coins:,} from {member.mention}'s balance.", user_id=uid, action_type='RemoveMoney')
...
async def drop_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await send_hybrid_error(ctx, content='❌ Missing arguments!\n**Usage:** `.drop <amount> [message]`\n**Example:** `.drop 5000 Enjoy the coins!`')
    elif isinstance(error, commands.BadArgument):
        await send_hybrid_error(ctx, content='❌ Invalid argument.\nUse formats like: `100`, `4k`, `2m`, `1.5mil`')
    elif isinstance(error, commands.CommandInvokeError):
        await send_hybrid_error(ctx, content='⚠️ Something went wrong while running this command.\nPlease try again later.')
    else:
...
async def ban(ctx, member: discord.Member, *, reason: str='No reason provided'):
    ...
    if err:
        return await ctx.send(err)
    embed = discord.Embed(title='⚠️ Confirm Ban', description=f'Are you sure you want to ban {member.mention}?', color=discord.Color.orange())
...
async def warn(ctx, member: discord.Member, *, reason='No reason provided'):
    guild_id = str(ctx.guild.id)
    user_id = str(member.id)
    warnings_data.setdefault(guild_id, {}).setdefault(user_id, []).append({'reason': reason, 'by': str(getattr(ctx.author, 'id', '')), 'time': datetime.now(timezone.utc).isoformat()})
    ...
    try:
        ...
        await member.send(f'⚠️ You have been **warned** in **{guild_name}**\n**Reason:** {reason}\n**Warned by:** {ctx.author} {author_mention}'.strip())
    except discord.Forbidden:
        try:
...
async def disable(ctx, target: str):
    ...
    await disabled_col.update_one({'guild': guild_id}, {'$set': {'disabled_commands': list(commands_set), 'disabled_categories': list(categories_set)}}, upsert=True)
...
class ModViewButtons(discord.ui.View):
    ...
    async def clear_warns(self, interaction: discord.Interaction, button: discord.ui.Button):
        await mod_col.update_one({'guild': str(self.ctx.guild.id), 'user': str(self.member.id)}, {'$set': {'warnings': []}})
    ...
    async def clear_specific(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message('🔢 Please enter the **number** of the punishment or note you wish to clear.\nExample: `1` to remove the first one.', ephemeral=True)
        ...
        await log_action(self.ctx, f'Cleared specific {key} for {self.member}: {entry_desc}', user_id=self.member.id, action_type='clearspecific')
...
class NoteModal(discord.ui.Modal, title='Add Moderator Note'):
    note = discord.ui.TextInput(label='Note Content', style=discord.TextStyle.paragraph, placeholder='e.g. Ban this user if he does it again', required=True, max_length=500)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await mod_col.update_one({'guild': str(ctx.guild.id), 'user': str(member.id)}, {'$push': {'notes': {'by': str(ctx.author), 'note': note_content, 'time': datetime.now(timezone.utc).isoformat()}}}, upsert=True)
...
class PerformanceView(discord.ui.View):
    ...
    def __init__(self, guild_id, staff_members, days=30):
        ...
        for member in staff_members[:25]:
            options.append(discord.SelectOption(label=member.display_name, description=f"Review {member.display_name}'s performance", value=str(member.id), emoji='👤'))
    ...
    async def dropdown_callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f'📊 Performance Review: {selected_member.display_name}', description=f'Analytics for {selected_member.mention}', color=discord.Color.blue())
        embed.add_field(name='📈 Basic Statistics', value=f"**Total Actions:** {analytics['total_actions']}\n**Messages Sent:** {analytics['total_messages']:,}\n**Commands Used:** {analytics['commands_used']:,}\n**Staff Since:** {analytics['staff_since']}", inline=False)
        ...
        embed.add_field(name='🕐 Activity Metrics', value=f"**Avg. Actions/Day:** {analytics['avg_actions_per_day']:.1f}\n**Most Active Day:** {analytics['most_active_day']}\n**Peak Hour:** {analytics['peak_hour']}:00\n**Active This Week:** {('Yes' if analytics['active_this_week'] else 'No')}", inline=False)
...
async def generate_performance_analytics(guild_id, staff_id, days=30):
    try:
        analytics = {'total_actions': 0, 'total_messages': 0, 'commands_used': 0, 'staff_since': 'Unknown', 'punishments': {'warn': 0, 'mute': 0, 'kick': 0, 'ban': 0, 'total': 0}, 'avg_actions_per_day': 0, 'most_active_day': 'Monday', 'peak_hour': 14, 'active_this_week': False, 'recent_activity': []}
        ...
        analytics['active_this_week'] = analytics['total_actions'] > 0 and any((doc.get('time') and parser.isoparse(doc['time']) >= seven_days_ago for doc in mod_data for doc_list in [doc.get('warnings', []), doc.get('mutes', []), doc.get('kicks', []), doc.get('bans', [])] for doc_item in doc_list if isinstance(doc_item, dict) and doc_item.get('by') == str(staff_id)))
        expected_daily = 2
        analytics['efficiency'] = min(100, analytics['avg_actions_per_day'] / expected_daily * 100)
        analytics['recent_activity'] = [f'Used {ptype} command' for ptype, count in analytics['punishments'].items() if ptype != 'total' and count > 0][:3]
        if not analytics['recent_activity']:
            analytics['recent_activity'] = ['No recent activity']
        return analytics
    except Exception as e:
        print(f'[Performance Analytics Error] {e}')
        return {'total_actions': 0, 'total_messages': 0, 'commands_used': 0, 'staff_since': 'Unknown', 'punishments': {'warn': 0, 'mute': 0, 'kick': 0, 'ban': 0, 'total': 0}, 'avg_actions_per_day': 0, 'most_active_day': 'Monday', 'peak_hour': 14, 'active_this_week': False, 'efficiency': 0, 'recent_activity': ['No data available']}
...
class ModerationConfirmView(discord.ui.View):
    ...
    async def confirm_no(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != (self.ctx.author.id if self.ctx else self.interaction.user.id):
            ...
            return
        self.confirmed = False
        embed = discord.Embed(title='❌ Moderation Cancelled', description=f'The {self.action} action on {self.member.mention} has been cancelled.', color=discord.Color.red())
    ...
    async def execute_moderation(self, interaction):
        try:
            ...
            if self.action == 'warn':
                await mod_col.update_one({'guild': str(ctx.guild.id), 'user': str(member.id)}, {'$push': {'warnings': {'by': str(ctx.author), 'reason': reason, 'time': datetime.now(timezone.utc).isoformat()}}}, upsert=True)
                try:
                ...
                if not mute_role:
                    ...
                    for ch in ctx.guild.channels:
                        await ch.set_permissions(mute_role, speak=False, send_messages=False)
                await member.add_roles(mute_role, reason=reason)
                await mod_col.update_one({'guild': str(ctx.guild.id), 'user': str(member.id)}, {'$push': {'mutes': {'by': str(ctx.author), 'reason': reason, 'time': datetime.now(timezone.utc).isoformat()}}}, upsert=True)
                if duration:
                    try:
                        seconds = parse_time(duration)
                        end_time = datetime.now(timezone.utc) + timedelta(seconds=seconds)
                        await mutes_col.update_one({'guild_id': ctx.guild.id, 'user_id': member.id}, {'$set': {'mute_end': end_time}}, upsert=True)
                ...
                await mod_col.update_one({'guild': str(ctx.guild.id), 'user': str(member.id)}, {'$push': {'kicks': {'by': str(ctx.author), 'reason': reason, 'time': datetime.now(timezone.utc).isoformat()}}}, upsert=True)
                ...
                await mod_col.update_one({'guild': str(ctx.guild.id), 'user': str(member.id)}, {'$push': {'bans': {'by': str(ctx.author), 'reason': reason, 'time': datetime.now(timezone.utc).isoformat()}}}, upsert=True)
                msg = f'✅ Banned {member.mention}.'
                await log_action(ctx, f'Ban executed on {member}: {reason}', user_id=member.id, action_type='ban')
            embed = discord.Embed(title=f'✅ {self.action.capitalize()} Executed', description=f'{msg}\n\nReason: {reason}' + (f'\nDuration: {duration}' if duration else ''), color=discord.Color.green())
            await interaction.response.edit_message(embed=embed, view=None)
            if self.message:
                punishments = await fetch_punishments(ctx.guild.id, member.id)
            ...
            error_embed = discord.Embed(title=f'❌ {self.action.capitalize()} Failed', description=f'An error occurred: `{type(e).__name__}: {e}`', color=discord.Color.red())
...
class WarnModal(discord.ui.Modal, title='Moderator Action'):
    ...
    def __init__(self, bot, ctx, member, action, message):
        ...
        self.message = message
        if self.action != 'warn':
            self.duration = discord.ui.TextInput(label='Duration (e.g., 1d 2h 7m; blank = permanent)', style=discord.TextStyle.short, required=False)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if member is None:
            try:
                ...
                return
        embed = discord.Embed(title=f'⚠️ Confirm {self.action.capitalize()}', description=f'Are you sure you want to {self.action} {member.mention}?', color=discord.Color.orange())
        embed.add_field(name='User', value=f'{member.mention} ({member.id})', inline=False)
        embed.add_field(name='Reason', value=reason, inline=False)
        if duration:
...
async def modview(ctx, member: discord.Member):
    ...
    verification_map = {VerificationLevel.none: 'None', VerificationLevel.low: 'Low', VerificationLevel.medium: 'Medium', VerificationLevel.high: 'High'}
...
async def stickynote(ctx):
    ...
    try:
        ...
        if doc:
            await sticky_col.update_one({'_id': doc['_id']}, {'$set': {'guild': str(ctx.guild.id), 'channel': str(ctx.channel.id), 'text': reply.content, 'message': sent.id}})
        else:
            await sticky_col.update_one({'guild': str(ctx.guild.id), 'channel': str(ctx.channel.id)}, {'$set': {'text': reply.content, 'message': sent.id}}, upsert=True)
...
async def on_member_join(member):
    ...
    try:
        ...
        if used_invite:
            ...
            await invites_col.update_one({'guild_id': str(guild.id), 'user_id': str(inviter.id)}, {'$inc': {'total': 1, 'regular': 1, 'joins': 1}}, upsert=True)
            await invites_col.update_one({'guild_id': str(guild.id), 'code': used_invite.code}, {'$set': {'inviter_id': str(inviter.id), 'uses': used_invite.uses}, '$addToSet': {'joined_users': str(member.id)}}, upsert=True)
        ...
        if member.premium_since:
            ...
            if not boost_record or boost_record.get('last_thanked') != member.premium_since.isoformat():
                ...
                if boost_ch:
                    boost_msg = doc.get('boost_message') or f'{member.mention} just boosted the pond! 🌟\nThank you for your support!'
                    text = boost_msg.replace('{username}', member.name).replace('{mention}', member.mention).replace('{server}', guild.name).replace('{boostcount}', str(guild.premium_subscription_count or 0))
                    boost_embed = discord.Embed(title='🚀 Boost Alert!', description=text, color=discord.Color.fuchsia(), timestamp=datetime.now(timezone.utc))
...
async def on_member_remove(member: discord.Member):
    ...
    if code_doc:
        inviter_id = code_doc.get('inviter_id')
        await invites_col.update_one({'guild_id': str(guild.id), 'code': code_doc.get('code')}, {'$pull': {'joined_users': str(member.id)}})
        if inviter_id:
            ...
            await invites_col.update_one({'guild_id': str(guild.id), 'user_id': str(inviter_id)}, {'$inc': {'leaves': 1}, '$set': {'total': max(joins - (leaves + 1), 0)}}, upsert=True)
...
async def quote(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title='💬 Random Quote', description=f'“{quote_text}”\n\n- *{author}*', color=discord.Color.purple())
        await ctx.send(embed=embed)
    except Exception as e:
...
async def onetime(ctx, channel: discord.TextChannel=None):
    ...
    if channel_id not in onetime_channels[guild_id]:
        ...
        await settings_col.update_one({'guild': guild_id}, {'$set': {f'onetime_channels.{channel_id}': {}}}, upsert=True)
        embed = discord.Embed(title='✅ One-Time Message Channel Set Up', description=f'**{target_channel.mention}** is now a one-time message channel.\n\nNon-staff members can send **only one message** in this channel. After their first message, they will lose permission to send more messages.\n\nStaff members are exempt and can continue messaging normally.\n\nUse `.restore <user>` to give a user back their messaging permissions.', color=discord.Color.green())
        await ctx.send(embed=embed)
        try:
...
async def restore(ctx, member: discord.Member, channel: discord.TextChannel=None):
    ...
    try:
        ...
        embed = discord.Embed(title='✅ Permissions Restored', description=f'{member.mention} can now send messages in {target_channel.mention} again.', color=discord.Color.green())
        await ctx.send(embed=embed)
    except Exception as e:
...
async def disableonetime(ctx, channel: discord.TextChannel=None):
    ...
    try:
        ...
        embed = discord.Embed(title='✅ One-Time Channel Disabled', description=f'{target_channel.mention} is no longer a one-time message channel.', color=discord.Color.green())
        await ctx.send(embed=embed)
    except Exception as e:
